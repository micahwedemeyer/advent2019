defmodule Day3 do
  ...
  def walk_directions({start_x, start_y}, [{direction, distance} | tail]) do
    ...
    new_start = Enum.at(path,0)
    walk_directions(new_start, tail) ++ path
  end

  def coord_list(_coords, _direction, 0), do: []
  def coord_list({start_x, start_y}, direction, 1), do: [step({start_x, start_y}, direction)]
  def coord_list({start_x, start_y}, direction, distance), do: step(coord_list({start_x, start_y}, direction, distance - 1), direction)

  def step([{x, y} | rest], direction), do: [step({x, y}, direction), {x, y}] ++ rest
